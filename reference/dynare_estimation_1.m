...
global M_ options_ oo_ estim_params_ dataset_ dataset_info
...
[dataset_, dataset_info, xparam1, hh, M_, options_, oo_, estim_params_, bounds, Prior, smoother_info, measurement_info] = ...
    dynare_estimation_init(var_list_, dname, [], M_, options_, oo_, estim_params_);

if options_.dsge_var
    check_dsge_var_model(M_, estim_params_);
...
try
    oo_ = initial_estimation_checks(objective_function,xparam1,dataset_,dataset_info,M_,estim_params_,options_,Prior,bounds,oo_,smoother_info);
    ...
    if options_.order==1 && ~options_.particle.status
        if options_.smoother
            if options_.occbin.smoother.status && options_.occbin.smoother.inversion_filter
                [~, info, ~, ~, ~, model_solution, smoother_results, oo_.occbin.smoother.regime_history] = occbin.IVF_posterior(xparam1,dataset_,dataset_info,options_,M_,estim_params_,Prior,prior_bounds(Prior,options_.prior_trunc),oo_.dr, oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state);
                if ismember(info(1),[303,304,306])
                    fprintf('\nIVF: smoother did not succeed. No results will be written to oo_.\n')
                else
                    smoother_results.model.mf = smoother_info.var_list(smoother_info.mf);
                    options_nk=options_.nk;
                    options_.nk=[]; %unset options_.nk and reset it later
                    [oo_]=store_smoother_results(M_,oo_,options_,smoother_info,dataset_,dataset_info,smoother_results);
            ...
            elseif learning.is_active(options_, 'social') && ...
                ...
                if options_.occbin.smoother.status
                    [smoother_results, occbin_info] = ...
                        occbin.DSGE_smoother(xparam1,M_,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,options_,estim_params_,dataset_,dataset_info,smoother_info);
                    if occbin_info.error_flag(1)==0
                        [oo_]=store_smoother_results(M_,oo_,options_,smoother_info,dataset_,dataset_info,smoother_results,occbin_info);
                    else
                        oo_.occbin.smoother.error_flag = occbin_info.error_flag;
                        oo_.occbin.smoother.regime_history = occbin_info.regime_history;
                        fprintf('\nOccBin: smoother did not succeed. No results will be written to oo_.\n')
                    end
                else
                    smoother_results = DsgeSmoother(xparam1,dataset_,dataset_info,M_,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,options_,estim_params_,smoother_info);
                    [oo_]=store_smoother_results(M_,oo_,options_,smoother_info,dataset_,dataset_info,smoother_results);
                end
                oo_.dr=smoother_results.model.dr; %required for subsequent call of forecasts.run
            end
...
if ~isequal(options_.mode_compute,0) && ~options_.mh_posterior_mode_estimation && ~issmc(options_)
    ...
    for optim_iter = 1:length(optimizer_vec)
        ...
        [xparam1, fval, ~, hh, Scale, new_rat_hess_info, optimization_info] = ...
            dynare_minimize_objective(objective_function,xparam1,current_optimizer,options_,[bounds.lb bounds.ub],Prior.name,[],hh,dataset_,dataset_info,options_,M_,estim_params_,Prior,bounds,oo_.dr, oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,smoother_info);
        ...
        if isnumeric(current_optimizer)
            ...
            elseif current_optimizer==6 %save scaling factor
                ...
                options_.jscale(:) = options_.mh_jscale;
            end
        end
            ...
            if options_.cova_compute == 1 %user did not request covariance not to be computed
                if options_.analytic_derivation && strcmp(func2str(objective_function),'dsge_likelihood')
                    ...
                    [~,~,~,~,hh] = feval(objective_function,xparam1, ...
                                         dataset_,dataset_info,options_,M_,estim_params_,Prior,bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,smoother_info);
                ...
                elseif ~isnumeric(current_optimizer) || ~(isequal(current_optimizer,5) && newratflag~=1 && strcmp(func2str(objective_function),'dsge_likelihood'))
                    ...
                    if options_.hessian.use_penalized_objective
                        ...
                        hh = hessian(penalized_objective_function, xparam1, options_.gstep, objective_function, fval, dataset_, dataset_info, options_, M_, estim_params_, Prior, bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,smoother_info);
                    else
                        hh = hessian(objective_function, xparam1, options_.gstep, dataset_, dataset_info, options_, M_, estim_params_, Prior, bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,smoother_info);
                    end
                    hh = reshape(hh, nx, nx);
                elseif isnumeric(current_optimizer) && isequal(current_optimizer,5)
                    ...
                    if compute_hessian
                        ...
                        hh = reshape(mr_hessian(xparam1,objective_function,fval,newratflag,crit,new_rat_hess_info,[bounds.lb bounds.ub],Prior.p2,0,dataset_, dataset_info, options_,M_,estim_params_,Prior,bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state), nx, nx);
                    end
                    options_.kalman_algo = kalman_algo0;
                end
            end
        end
        parameter_names = Prior.name;
    end
...
if options_.mode_check.status && ~options_.mh_posterior_mode_estimation && ~issmc(options_)
    ...
    mode_check(objective_function,xparam1,hh,options_,M_,estim_params_,Prior,bounds,false,...
               dataset_, dataset_info, options_, M_, estim_params_, Prior, bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state);
    options_.analytic_derivation = ana_deriv_old;
end
...
if ~issmc(options_) && Prior.bayes() && ~options_.mh_posterior_mode_estimation
    % display results table and store parameter estimates and standard errors in results
    oo_ = display_estimation_results_table(xparam1, stdh, M_, options_, estim_params_, oo_, prior_dist_names, 'Posterior', 'posterior', Prior);
    % Laplace approximation to the marginal log density:
    if options_.cova_compute
        estim_params_nbr = size(xparam1,1);
        if ispd(invhess)
            log_det_invhess = log(det(invhess./(stdh*stdh')))+2*sum(log(stdh));
            likelihood = feval(objective_function,xparam1,dataset_,dataset_info,options_,M_,estim_params_,Prior,bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,smoother_info);
    ...
    if options_.dsge_var
        [~,~,~,~,~,dsge_var_model_solution] =...
            feval(objective_function,xparam1,dataset_,dataset_info,options_,M_,estim_params_,Prior,bounds,oo_.dr,oo_.steady_state,oo_.exo_steady_state,oo_.exo_det_steady_state,smoother_info);
        oo_.dsge_var.posterior_mode.PHI_tilde = dsge_var_model_solution.PHI_tilde;
        oo_.dsge_var.posterior_mode.SIGMA_u_tilde = dsge_var_model_solution.SIGMA_u_tilde;
        oo_.dsge_var.posterior_mode.iXX = dsge_var_model_solution.iXX;
        oo_.dsge_var.posterior_mode.prior = dsge_var_model_solution.prior;
    end
elseif ~issmc(options_) && ~Prior.bayes() && ~options_.mh_posterior_mode_estimation
    oo_=display_estimation_results_table(xparam1, stdh, M_, options_, estim_params_, oo_, prior_dist_names, 'Maximum Likelihood', 'mle', Prior);
end

if ~issmc(options_)
    invhess = set_mcmc_jumping_covariance(invhess, nx, options_.MCMC_jumping_covariance, 'dynare_estimation_1', Prior);
end
...
if issmc(options_)
    [posterior_sampler_options, options_] = check_posterior_sampler_options([], M_.fname, M_.dname, options_, bounds, Prior);
    options_.posterior_sampler_options.current_options = posterior_sampler_options;
    if isonline(options_)
        online_auxiliary_filter(xparam1, dataset_, options_, M_, estim_params_, Prior, oo_);
    elseif ishssmc(options_)
        oo_.MarginalDensity.hssmc = hssmc(objective_function, bounds, dataset_, dataset_info, options_, M_, estim_params_, Prior, smoother_info, oo_);
    elseif isdime(options_)
        dime(objective_function, xparam1, bounds, dataset_, dataset_info, options_, M_, estim_params_, Prior, smoother_info, oo_.dr, oo_.steady_state, oo_.exo_steady_state, oo_.exo_det_steady_state);
    elseif isdsmh(options_)
        oo_.MarginalDensity.dsmh = dsmh(objective_function, bounds, dataset_, dataset_info, options_, M_, estim_params_, Prior, smoother_info, oo_);
    end
end
...
if issmc(options_) || (Prior.bayes() && options_.mh_replic) ||  (Prior.bayes() && options_.load_mh_file) % not ML estimation
    if ~issmc(options_)
        % Reset bounds as lb and ub must only be operational during mode-finding
        bounds = set_mcmc_prior_bounds(xparam1, Prior, options_, 'dynare_estimation_1');
        ...
        if options_.mh_replic || options_.load_mh_file
            ...
            [posterior_sampler_options, options_] = check_posterior_sampler_options(posterior_sampler_options, M_.fname, M_.dname, options_, bounds, Prior);
            % store current options in global
            options_.posterior_sampler_options.current_options = posterior_sampler_options;
            if options_.mh_replic
                ana_deriv_old = options_.analytic_derivation;
                options_.analytic_derivation = false;
                posterior_sampler(objective_function,posterior_sampler_options.proposal_distribution,xparam1,posterior_sampler_options,bounds,dataset_,dataset_info,options_,M_,estim_params_,Prior,oo_,dispString,smoother_info);
                options_.analytic_derivation = ana_deriv_old;
            end
        end
    ...
    if options_.mh_posterior_mode_estimation || (issmc(options_) && options_.smc_posterior_mode_estimation)
        [~, covariance, posterior_mode] = compute_posterior_covariance_matrix(Prior.name, M_.fname, M_.dname, options_);
        ...
        elseif isdime(options_)
            ...
            [~, ~, posterior_mode] = compute_posterior_covariance_matrix(Prior.name, M_.fname, M_.dname, options_);
        ...
        if isdsmh(options_) || ishssmc(options_) || isonline(options_) || isdime(options_) || options_.mh_replic || (options_.load_mh_file && ~options_.load_results_after_load_mh)
            if ~issmc(options_)
                [~, oo_] = marginal_density(M_, options_, estim_params_, oo_, 'Output', Prior);
            end
            % Store posterior statistics by parameter name
            oo_ = GetPosteriorParametersStatistics(estim_params_, M_, options_, oo_, prior_dist_names, Prior);
            if ~options_.nograph
        ...
        if ~(~isempty(options_.sub_draws) && options_.sub_draws==0)
            if options_.bayesian_irf
                ...
                if options_.occbin.likelihood.status
                    ...
                    oo_=PosteriorIRF('posterior',options_,estim_params_,oo_,M_,Prior,dataset_,dataset_info,dispString);
                end
            end
            ...
            if options_.smoother || ~isempty(options_.filter_step_ahead) || options_.forecast
                ...
                if options_.order==1 && ~options_.particle.status
                    oo_=prior_posterior_statistics('posterior',dataset_,dataset_info,M_,oo_,options_,estim_params_,Prior,smoother_info,dispString); %get smoothed and filtered objects and forecasts
                else
...
if options_.smoother && ... %Bayesian smoother requested before
    (Prior.bayes() && options_.mh_replic || ... % Bayesian with MCMC run
    Prior.bayes() && options_.load_mh_file) % Bayesian with loaded MCMC
    % nothing to do
